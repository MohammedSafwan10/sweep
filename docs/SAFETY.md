...
6. **Sizes are re-statted at delete time.** Byte totals describe logical file lengths, not guaranteed disk space recovered. Hard links and sparse files can differ; moving to the Recycle Bin does not release disk space until it is emptied.
7. **Confirmation executes the displayed plan.** It does not discover and delete new findings after approval. Duplicate paths are counted once; overlapping parent findings are skipped in favor of more specific paths.
